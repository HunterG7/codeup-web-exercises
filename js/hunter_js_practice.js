(() => {
    ...
    // //     let fullArray = strArray;
    // //     for (let i = 0; i < fullArray.length; i++) {
    // //         let char = strArray[i];
    // //         console.log(char);
    // //         strArray = strArray.splice(i, 1);
    ...
    // let oranges = fruits[2];
    // let numOfFruits = fruits.length;
    // console.log(`The number of fruits are ${numOfFruits}.`);
    //
    // names = ["Brian", "Bill", "Dak"];
...
//to access object properties, use dot notation
    //3console.log(currentWeather.humidity);
...
// command ctrl g
    ...
    hourlyWeather.forEach(hourlyForecast => console.log(hourlyForecast.temperature)); //arrow forEach

    hourlyWeather.forEach(function(hourlyForecast) { // classic forEach
        console.log(`At ${hourlyForecast.time}, the temperature will be ${hourlyForecast.temperature}.`);
    });

    for (let i = 0; i < hourlyWeather.length; i++) { // for loop
        console.log(`At ${hourlyWeather[i].time}, the temperature will be ${hourlyWeather[i].temperature}.`)
    }

    for (const forecast of hourlyWeather) { // for of loop: used to loop through an array
        console.log(forecast.temperature);
    }
    const name = "Hunter";
    for (const letter of name) console.log(letter);

    // a for each loop only loops through arrays
